<?php
	...
	if(isset($_POST["register"]))
	{
		...
		$domain = substr($email, strpos($email, '@') + 1);
			...
			$sqlUname = "select * from signedup WHERE fname = :fname OR lname = :lname";
			$command2=$connect->prepare($sqlUname);
			$command2->bindValue(":fname", $fname);
			$command2->bindValue(":lname", $lname);
			$command2->execute();
			$result2 = "";
		...
		elseif($result2 == "Létezik!")
		{
			echo "<p class='p3'>Felhasználó név már létezik!</p>";
		}
		else
		{
