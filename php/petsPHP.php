<?php
			...
			if(isset($_SESSION["email"]))
			{
				...
				<input value='$id' type='hidden' name='favorite'>
					...
					<div class='pet-advertisement-caption'>
						...
						<a href='petDescription.php?id=$id'><img class='img10' src='../images/animals/$image' alt='hírdetés'></a>
					</div>";
					if($i==2)
					{
						echo "<h6></h6>";
	...
	{
			$sqlCommand2="SELECT * FROM pets INNER JOIN category ON pets.categoryId = category.categoryId INNER JOIN status s ON pets.statusId = s.statusId";
			$sqlQuery2=$connect->prepare($sqlCommand2);
			$sqlQuery2->execute();
			$i=0;
			$count=0;
