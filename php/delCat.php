...
<body>
	...
	?>
	<div class="container-fluid div3"><a class='anchor'  href='adminLoggedIn.php'>Kategória Törlése</a></div>
	<div class='modify-categories'>
		<form method='post' action='delCat.php'>
		<?php
		if(isset($_POST["deleteCategory"]))
		{
			if(($_POST["animalSelect"]) != '')
			{
				$type=$_POST["animalSelect"];
				$sql = "DELETE FROM category WHERE categoryId = :cId";
				$q1 = $connect->prepare($sql);
				$q1->bindValue(":cId",$type);
				$q1->execute();
				echo "<br><b class='par1'>Sikeresen törölve!</b><br>";
			}
			else
			{
		...
		<label for="lbl-anml1"><b>Állat fajtája</b></label><br>
			...
			<input type='submit' value='Kategória Törlése' class='btn btn-primary btn-lg btn-block' name='deleteCategory'>
		</form>
